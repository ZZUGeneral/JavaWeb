...
public class UpdateStudentScoreServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (i > 0) {
			request.getRequestDispatcher("/ShowStudentScoreServlet?pageNo=false").forward(request, response);
		} else {
			PrintWriter out = response.getWriter();
			out.print("<script type='text/javascript'>");
			out.print("alert('打分失败！');");
			out.print("window.location='ShowStudentScoreServlet?pageNo=false'");
			out.print("</script>");
		}
	}
